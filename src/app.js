...
const routes = [
  {
    ...
    view: async () => {
      ...
      new AuthPresenter(
        authModel,
        { bindLogin: view.bindLogin, showError: view.showError },
        router
      );
      return view.getView();
    },
        ...
        storyModel,
        {
          bindLoadStoryDetail: (handler) => {
            const mapContainer = view
              .getView()
              .querySelector("#story-map-container");
            if (mapContainer) {
                ...
                if (!mapElement || !mapElement._leaflet_id) {
                  try {
                    ...
                    L.marker([story.lat, story.lon])
                      .addTo(map)
                      .bindPopup(
                        `<b>${story.name}</b><br>${story.description.substring(
                          0,
                          50
                        )}...`
                      )
      ...
      try {
        const mapPresenter = new MapPresenter({
          bindInitMap: async (handler) => {
            const result = await initMapWithRetry(
              handler,
              "map",
              -6.2088,
              106.8456,
...
if ("serviceWorker" in navigator) {
  ...
  navigator.serviceWorker.addEventListener("message", (event) => {
    if (event.data?.type === "TRIGGER_SYNC") {
      if (
        window.storyPresenter &&
        typeof window.storyPresenter.handleOnline === "function"
      ) {
